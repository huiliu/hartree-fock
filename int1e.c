...
double R(int n, int t, int u, int v, const gsl_vector *PX, double gamma, FMW *fmw, int debug)
{
// L. E. McMurchie, R. Davidson J. comp. pyhy. 26, 218 (1978)
// (4.6) (4.7) (4.8)
    double finc = 0;
    ...
    if (t == 0 && u == 0 && v == 0) {
        ...
        if (n > fmw->count) {
            ...
            for (i = 9; i >= 0; i++) {
                rb = &fmw->RBList[n-i];
                if( !(rb = rbinit(compare, NULL))) {
                    fprintf(stderr, "redblack allocate memory error!\n");
                    exit(EXIT_FAILURE);
                }
            }
        }

        item1 = F_inc_gamma(n, finc, &fmw->RBList[n]);
        return gsl_pow_int(-2*gamma, n) * item1;
#else
        // don't use red-black tree store and search F_inc_gamma value
        return gsl_pow_int(-2*gamma, n) * F_inc_gamma(n, finc, NULL);
#endif
        //return gsl_pow_int(-2*gamma, n) * F_inc_gamma(n, finc, f);
    }
    if (t > 0) {
        if (t-1 == 0)
            item1 = 0;
        else
            item1 = (t-1) * R(n+1, t-2, u, v, PX, gamma, fmw, debug);
        if (PX->data[0] == 0)
            item2 = 0;
        else
            item2 = PX->data[0] * R(n+1, t-1, u, v, PX, gamma, fmw, debug);
    }
    if (u > 0) {
        if (u - 1 == 0)
            item1 = 0;
        else
            item1 = (u-1) * R(n+1, t, u-2, v, PX, gamma, fmw, debug);
        if (PX->data[1] == 0)
            item2 = 0;
        else
            item2 = PX->data[1] * R(n+1, t, u-1, v, PX, gamma, fmw, debug);
    }
    if (v > 0) {
        if (v-1 == 0)
            item1 = 0;
        else
            item1 = (v-1) * R(n+1, t, u, v-2, PX, gamma, fmw, debug);
        if (PX->data[2] == 0)
            item2 = 0;
        else
            item2 = PX->data[2] * R(n+1, t, u, v-1, PX, gamma, fmw, debug);
    }
    return item1 + item2;
}
...
double overlap_gto(const GTO* g1, const gsl_vector* A, const GTO* g2, const gsl_vector* B, int debug)
{
    ...
    result = RecCoeff(l1, l2, 0, &gamma, PA->data,   PB->data) * \
             ...
             RecCoeff(n1, n2, 0, &gamma, PA->data+2, PB->data+2);
    result *= pow(M_PI/gamma, 1.5) * norm1 * norm2 * c1 * c2 * KAB;
    gsl_vector_free(PA);
    gsl_vector_free(PB);
    return result;
}
    ...
    for (i = 0; i <= L; i++) {
        ...
        for (j = 0; j <= M; j++) {
            ...
            for (k = 0; k <= N; k++) {
                ...
                result += Ex * Ey * Ez * R(0, i, j, k, PC, gamma, fmw, debug);
            }
        }
    }
    gsl_vector_free(PA);
